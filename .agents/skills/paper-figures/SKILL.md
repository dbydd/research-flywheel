...
- 弱式反例：`a: R²` → `b: R² 两两比较` → `c: MAPE` → `d: MAPE 两两比较`，四个面板往往只有两种推理角色。强式序列：扰动设计 → 决定性对照 → 全分布或残差 → 分解或边界。
...
|---|---|---|---|---|---|---|---|---|
| a | 只有这块回答的那个问题 | mean/median/raw | SD/SE/CI/none + 理由 | seeds/folds/slices/subjects | 最终显示标签 | 行列分组、偏差或豁免 | 审计发现 + 数据与误差极值 + 文字 bbox | yes/no |

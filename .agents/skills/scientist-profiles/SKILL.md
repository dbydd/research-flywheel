...
`代表论文` 链接指向 `papers/abstracts/` 下的摘要页，`主要合作者` 链接指向其他人物页；目标页还没建时直接留一个未解析的 `[[链接]]`（它就是待建标记），本页照写链，lint 的「断链」条目会收集它们，补页后自然消掉。
...
孤点是没有任何入库合著边的入库学者。孤点要么回采集阶段补采放宽阈值，要么从库里摘掉并记原因。摘人与补采记进自己 ws 的 `STATE.md` 一行，原因写进相关页或快照本身。

## 落账

- 采集快照写 `raw/<YYYY-MM-DD>-<seed 短名>.collection.json`：seeds、所用超参、种子作者 id、候选数、缓存作品数、合著频次前若干。快照只增不改。
- 本跳读数（入库人数、按层分布、边数、孤点数、快照路径）写进自己 ws 的 `STATE.md` 一行；机器账在 onlyne ledger，仓里没有流水文件。
- 人物页的落点就是 `people/` 目录与图谱；建页与改动同步 frontmatter 的 `created`/`updated`，导航不设全库目录页。
- 拆分、合并、改目录等结构动作照 `wiki-format` 执行，改动痕迹由 git 历史承载。

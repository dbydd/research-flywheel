...
- 仓根不放知识记录文件。流水归三处：机器账在 onlyne ledger（每跳任务书、回执、残差流逐帧可回放），时刻在页面自身（frontmatter `created`/`updated`），过程在各 role 的 ws `STATE.md`。中央追加式账本在几十部文献的规模下必然腐成噪音堆，还会用陈旧流水误导后来者——不留。
- 机器件（`.onlyne/`、`.pi/`、`.agents/`、`.supervisor/`、`tools/`）不属知识层，本格式不管它们。工具区只放脚本、工具与临时文件，不写知识笔记。
- 任务书是瞬态交接面，不落盘不存档；仓里没有任务书目录。过程与结论落在各 role 自己的 ws `STATE.md`，机器侧流水在 onlyne ledger。
...
- 拆分、合并、改目录是常规结构动作，随时可做；落点写进相关页面的 frontmatter 与各自 ws `STATE.md`，改动痕迹由 git 历史承载。
...
引用纪律（总律=可擦性）：报告是连贯的文章，读者从头到尾不需要打开第二个文件。正文里任何外部指称记号——他文的 § 节号、Table/Figure、公式号、PDF 页码、`[[slug]]`、裸编号——从句子里擦掉，句义不许少一分；承载理解的引用是欠条，只能在句中兑现：原文为证的段落整段嵌入或归纳转述。擦得掉的记号还要能就地兑现给只拿着这一页的读者：内指本页的编号须与页面真实标题对得上；外指他文的必须冠名（「《DeepSeek-V4》§3.2.1」，不是光秃的「（§3.2.1）」）；链接指到仓里真实存在的东西。无出处的裸编号、点不开的链接、擦不掉的记号一律悬空，满页如此就是未完成品的形状，对质位打回。抽取文本的行号是机器对账坐标，读者永远兑现不了，禁入正文。配额让位于判据：不为「少挂」而把该嵌的内容删掉，也不因「允许挂」就往句子里塞坐标。图与表格豁免：正文自绘表格与 `![[…]]` 嵌页图是页面自身的内容，照常有、照常用，引用它们的编号不算悬空——本律只管读者兑现不了的外部记号。
...
<每条证据自明：数字改写成人话陈述，原文为证的整段嵌入或归纳转述；句尾的溯源记号必须擦得掉且就地可兑现（外指冠名、内指真标题），不写「见行 xxx」「详见 §光杆」>
...
- 可擦性与就地兑现：位置记号（§节号、Table/Figure、公式号、PDF 页码）只作溯源、擦掉不减句义，且外指必冠名、内指必对得上本页真实标题；读者顺着正文就能读完，一次都不用跳。抽取文本行号永不入正文；引原文就整段嵌入或归纳转述。
...
本格式管知识笔记。role 记事另有一套纪律，住在各层 `STATE.md` 手帐面：不写时间戳，记叙段加条目段，手记。两套互不搬：知识笔记的时刻只写在 frontmatter，记事里只写过程与指针。注入面只放固定物，`AGENTS.md` 链只容纳身份、岗位口径、约定纪律与文件指针。

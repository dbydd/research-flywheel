...
引用纪律（总律=可擦性）：报告是连贯的文章，读者从头到尾不需要打开第二个文件。正文里任何记号——§ 节号、Table/Figure、公式号、PDF 页码、`[[slug]]`、裸编号——从句子里擦掉，句义不许少一分；承载理解的引用是欠条，只能在句中兑现：原文为证的段落整段嵌入或归纳转述。擦得掉的记号还要能就地兑现给只拿着这一页的读者：内指本页的编号须与页面真实标题对得上；外指他文的必须冠名（「《DeepSeek-V4》§3.2.1」，不是光秃的「（§3.2.1）」）；链接指到仓里真实存在的东西。无出处的裸编号、点不开的链接、擦不掉的记号一律悬空，满页如此就是未完成品的形状，对质位打回。抽取文本的行号是机器对账坐标，读者永远兑现不了，禁入正文。配额让位于判据：不为「少挂」而把该嵌的内容删掉，也不因「允许挂」就往句子里塞坐标。

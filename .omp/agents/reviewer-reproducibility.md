...
Read the full `traces/<run_id>/` envelope and `reports/report.md`. Then verify executable reproducibility from a clean process (portable default: `uv run --frozen python`, no OMP `eval` required):
...
Output: `score` (0-5), `findings` (numbered, blocking|major|minor), `verdict` (accept|revise|reject), `measured_metric` (your own re-run value), `matches_published` (true|false). Score 0-2 reject, 3 revise, 4-5 accept. `bash` is for executing `uv sync --frozen` and `uv run --frozen python` / `reproduce.sh` in fresh processes only; never modify workspace files; never require an OMP persistent kernel. A failed personal reproduction is a blocking finding regardless of the other reviewers' scores.

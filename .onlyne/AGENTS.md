...
|`.onlyne/ws/gemini/<role>/AGENTS.md`|我的私有记事：记叙段写过程，条目段写索引|只有该 role 自己写|
...
记叙段用完整的话说事：这一摊是什么、我干了什么、为什么这么干、手里有什么结论、哪里还悬着。读一遍能接上，不用翻别处。

条目段一条一行：标题加路径。标题讲清那是什么东西，路径指到承载它的文件，要细节顺着路径翻。
...
subagent 随便起：`Agent` 后台跑，`get_subagent_result` 取结果，`steer_subagent` 中途拨方向。网随便搜：`web_search`、`fetch_content`、`source_check`。两个 role 一个待遇，能拆出去的活拆出去，能查到的先查。

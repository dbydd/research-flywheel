...
4. 图表：数据图用 matplotlib 脚本绘制，脚本与成图落 `papers/<项目短名>/figs/`，数值从 measured/ 读取；示意图以文本源（mermaid 或 tikz）为正典，源文件同目录留存。每张子图按 paper-figures「逐面板 QA」一节逐项检查后再交，检查过即可，不留检查单据。
...
- 任务书三段、草稿先落自己 ws 的 `work/`、dispatch.md 存档，均按仓根约定。

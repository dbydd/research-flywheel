...
大循环与关口定义见仓根 AGENTS.md：按 设计段 实验设计段落地实测，填结果节。

## 职责
- 唯一输入 = `<slug>.md` 设计段 实验设计段 + 断言清单。spec.md 已废止。
- 对设计只提可执行性意见（回 speculator），不改设计正文。结果节本角色填。

## 输入
- `research_project/<slug>.md` 设计段 实验设计（六件套）与断言清单。
- qa 返工令。run-id=`<slug>--<轮次>`。

## 期望产物
- `runs/<run-id>/measured/` 与 `summary.md`、`environment.json`、`assertions.json`。
- `<slug>.md` 设计段 结果节（只增五位）：原始读数表(每数字后缀 measured/ 路径)｜终态断言对表(assertions.json)｜环境行(wall-time/峰值内存/therm)｜有效读数判定(无效格子点名+原因)｜delta vs baseline。
...
## 纪律
- 段权：设计段 结果只增不改；revise 只改本段；落笔在 run-log.md 记一行。

...
## 职责
- 唯一输入 = `<项目短名>.md` 设计段实验设计段 + 断言清单。spec.md 已废止。
- 对设计只提可执行性意见（回 speculator），不改设计正文。结果节本角色填。

## 输入
- `research_project/<项目短名>.md` 设计段实验设计与断言清单。
- qa 返工令。run-id=`<项目短名>--<轮次>`。

## 期望产物
- `runs/<run-id>/measured/` 与 `summary.md`、`environment.json`、`assertions.json`。
- `<项目短名>.md` 设计段结果节（只增不改）：读数表每数字后缀 measured/ 路径、断言对表指 assertions.json、环境行、无效读数点名、delta vs baseline。行文自由，数字来源是硬线。

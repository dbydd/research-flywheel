...
大循环与关口定义见仓根 AGENTS.md：带形式化陈述出假设与方案，主笔 假设段 与 设计段 设计。

## 职责
- 提出解决方案与理论论据。假设段 假设+论据主笔；设计段 实验设计主笔（runner 只提可执行性意见）。
...
## 期望产物
- `research_project/<slug>.md` 假设段：假设（五字段）+ 理论论据（四件套）+ 挂 theorist 形式化小节。
- 同文件 设计段 实验设计（六件套）+ 断言清单，作为 runner 唯一输入。
...
## 纪律
- 段权：假设段 与 设计段 设计只增不改；revise 只改本阶段段；落笔在 `runs/<run-id>/run-log.md` 记一行。

# theorist —— 形式化官（D3 theory · `.onlyne/templates/formal/theory/theorist/`）

大循环与关口定义见仓根 AGENTS.md：G1 pass 后先形式化「最终验证依据」，再交 speculator。

## 职责
- 形式化官。收 examiner 开工令，先把最终验证依据写成可 Lean 的命题。
- 产物 = Lean 文件 + 陈述对照，挂 `<slug>.md` §1 形式化小节。不写 spec.md，不派 runner。
...
## 期望产物
- `runs/<run-id>/header-snapshot.md`：开工时冻结头四段快照（含最终验证依据）。run-id=`<slug>--r<round>`。
- `runs/<run-id>/lean/`：命题编译过；无新断言则 inherited pass。
- `<slug>.md` §1 形式化小节：Lean 路径 + 陈述↔论据对照表。
...
## 纪律
- 段权：§1 形式化小节只增不改；revise 只改本段；落笔在 `runs/<run-id>/run-log.md` 记一行。

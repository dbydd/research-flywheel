...
- Shell entrypoints (`reproduce.sh`, `orchestration/hooks/*.sh`) prefer `uv` when `uv` and `uv.lock` exist (`uv run --frozen python ...`), with `python3`/`python` fallback for bootstrap portability. Formal gates (compile/smoke/keeper reproduction) always run as fresh `uv run --frozen python` subprocesses.
- Documentation and templates call `uv sync --frozen` once and then `uv run --frozen python ...` for every Python invocation.

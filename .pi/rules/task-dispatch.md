...
- **Session affinity.** Fresh by default; resume only per `.pi/rules/session-affinity.md` (in-phase iteration / consecutive small steps / incremental deepening); 2 consecutive rejects on the same artifact force a fresh session with a re-framed prompt; review/audit roles never resume anyone. Every dispatch appends to `runs/<run_id>/sessions.jsonl`.

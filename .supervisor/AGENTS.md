...
- 进会话先读两层正本：仓根 `AGENTS.md`（共享目标：记叙段的主线与判据、条目段的支线与材料）与 `.onlyne/AGENTS.md`（角色行为约定：一跳、任务书四段、记事纪律、手上的家伙）。omp 会话开起来时 `.omp/APPEND_SYSTEM.md` 已经把这句话带进系统提示。
- 集群运维口径的正本是 `README.md`。本文件管值班。
- 我只做值班与记账。我亲自写的文件限于仓根 `AGENTS.md`。任务书当场写进命令，不在仓里存档。领域工作派给 worker。

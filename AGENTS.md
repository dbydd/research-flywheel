...
Dispatch protocol: each subagent `task` call passes `agent: "<project-agent-name>"` with the full context the subagent needs (run_id, idea record, trace paths, scout evidence index, output contract from `.pi/agents/<name>.md`). Subagents start blank and never see this chat. `modeler` always runs with `isolation: "worktree"`. The three reviewers run in parallel (one batch); never serialize independent reviews. Large artifacts stay in files; prompts carry paths, never blobs.
...
2. **Investigate**: if `research/` lacks evidence for this idea's method domain, dispatch `literature-scout` (frontier survey + local artifact pull + provenance). Do not skip this step for a "quick" run — quick uninspected runs are the failure mode.
...
7. **Audit**: dispatch `analyst` on `verification.json`. `audit_fail` → treat as discard with findings preserved.

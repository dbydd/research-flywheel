...
> 这是一棵 Research Flywheel 模板树：五角色（scout 侦察 → model 推导 → bench 测量 → writer 成稿 → critic 审稿）的自转科研环，任务经 onlyne 集群投递，知识全落文件。我能替你把它装配成一个属于你主题的运行实例。三个问题：
> 1. **主题是什么**？一句话研究问题 + 你手头有什么材料（论文/数据/代码/想法笔记，给路径）。没有明确主题也可以——说一声，我们先跑一轮文献侦察把它问出来。
> 2. **装在哪、跑给谁**？就在当前这台机器跑，还是先只要装配好的分支不运行？
> 3. **算力边界**？本机跑批有无功耗/时段约束（决定 bench 的时间片与训练槽条款怎么写）。

用户只想浏览时，给出三句话全景（模板=骨架 / theme 分支=实例 / 通电=跑环），停在 §0 全景图，不推进任何文件改动。

依赖安装归 §2（工具链三步，无发布渠道是新人第一坑，遇到先核 PATH）；§5 九检全绿才算装配完成，中途任何一检失败按 §8 常见坑对表自修，修不动再向用户报告卡点。

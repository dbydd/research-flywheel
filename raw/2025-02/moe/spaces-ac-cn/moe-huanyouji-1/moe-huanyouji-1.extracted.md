...
> 1、一个常规的Dense模型FFN，可以等价改写为 $n$ 个Expert向量 $\boldsymbol{v}_1,\boldsymbol{v}_2,\cdots,\boldsymbol{v}_n$ 之和；2、为了节省计算量，我们试图挑出 $k$ 个向量求和来逼近原本的 $n$ 个向量之和；3、转化为数学问题求解后，我们发现挑选规则是模长最大的 $k$ 个向量；4、直接去算 $n$ 个Expert的模长然后选 $k$ 个实际上是不省计算量的，所以要重新设计Expert；5、将 $\boldsymbol{v}_i$ 归一化得到 $\boldsymbol{e}_i$ ，然后用另外的小模型（Router）预测模长 $\rho_i$ ，最终的Expert为 $\rho_i \boldsymbol{e}_i$ ；6、此时，我们就可以先算全体 $\rho_i$ ，挑出 $k$ 个后才去计算 $\boldsymbol{e}_i$ ，达到节省计算量的目的。
...
***转载到请包括本文地址：**[https://spaces.ac.cn/archives/10699](https://spaces.ac.cn/archives/10699)*
...
苏剑林. (Feb. 08, 2025). 《MoE环游记：1、从几何意义出发 》[Blog post]. Retrieved from [https://spaces.ac.cn/archives/10699](https://spaces.ac.cn/archives/10699)

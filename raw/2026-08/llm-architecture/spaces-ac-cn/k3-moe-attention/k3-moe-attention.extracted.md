...
在这些东西之中，AttnRes我们之前已经在[《Attention Residuals 回忆录》](/archives/11664)做过详细介绍，而KDA我们也已经分享过详细的技术报告[《Kimi Linear: An Expressive, Efficient Attention Architecture》](https://papers.cool/arxiv/2510.26692)。至于Per-Head Muon，它其实没有效果上的优势（当然也没有劣势），改用它更多是出于正确性考虑（每个Head本就是相对独立的，不应耦合在一块）。
...
$$
\newcommand{SiLU}{\mathop{\text{SiLU}}}\boldsymbol{W}_3(\SiLU(\boldsymbol{W}_1 \boldsymbol{x}) \odot \boldsymbol{W}_2 \boldsymbol{x})
$$\
...
$$
\newcommand{SiTU}{\mathop{\text{SiTU}}}\newcommand{softcap}{\mathop{\text{softcap}}}\SiTU(x;\beta) = \underbrace{\beta \tanh\left(\frac{x}{\beta}\right)}_{\softcap(x;\beta)}\cdot\sigma(x)
$$\
这样先将门控部分控制在$(-\beta, \beta)$内，其中$\beta=4$。进一步压测发现，这样还不能完全杜绝膨胀，所以我们干脆把线性也加上了$\softcap$运算，形成了如今的SiTU-GLU：\
$$
\boldsymbol{W}_3\Big(\SiTU(\boldsymbol{W}_1 \boldsymbol{x};\beta_1) \odot \softcap(\boldsymbol{W}_2 \boldsymbol{x};\beta_2)\Big)
$$\
...
$$
\text{MoE:}&\qquad\qquad\underbrace{d \to D \to d}_{n\text{选}k} \\[5pt]
...
跟上一代模型K2一样，K3同样采用[Loss-Free](/archives/10757)的负载均衡方案，但之前所用的SignSGD式更新规则，在K3这么大的总Expert数下表现已经不大稳定了，所以我们引入了QB（Quantile Balancing），优点是数学上更合理，且无额外超参，其细节我们在[《MoE环游记：6、最优分配促均衡》](/archives/11619)已做过详细介绍。
...
![image](/usr/uploads/2026/05/2013347987.png)
...
一年前，笔者写了[《Transformer升级之路：20、MLA好在哪里?（上）》](/archives/10907)和[《Transformer升级之路：21、MLA好在哪里?（下）》](/archives/11111)，从实验和理论来探究MLA的好处，当时笔者给出的判断是：“**在相同训练成本和推理成本下，MLA可能是效果最好的Full Attention变体。**”。
...
在[《Transformer升级之路：20、MLA好在哪里?（上）》](/archives/10907)中，我们实验发现无形增大的head_dims是MLA效果的关键，然后在[《Transformer升级之路：21、MLA好在哪里?（下）》](/archives/11111)中，我们指出给定KV Cache大小下，效果最好的Attention是“一个head_dims等于KV Cache大小、K和V共享的MQA”。

于是，DSV4直接将Attention换成head_dims=512、K=V的MQA（位置编码是[QKVO-RoPE](/archives/10862)），以保证效果，而这正是MLA的Decoding形式。但这样一来，训练和Prefill的计算暴涨，且DSV4没有线性注意力，每一层都有KV Cache，即便每Token只有512维也吃不消。为此，DSV4引入了Sparse和Compress：Sparse节省计算量，Compress进一步压缩KV Cache，也节省计算量。
...
为什么“KDA+MLA”就可以NoPE了呢？我们在[《Transformer升级之路：6、旋转位置编码的完备性分析》](/archives/9403)推导过，任意正交矩阵的幂都可以用作构建广义的RoPE，通常说的RoPE选择的是简单的旋转矩阵，而[PaTH](https://papers.cool/arxiv/2505.16381)尝试了另一种选择——Householder矩阵，并且表现也不错。

在[《线性注意力简史：从模仿、创新到反哺》](/archives/11033)中我们推导过，在正交情形下，PaTH可以等价写成\
...
***转载到请包括本文地址：**<https://spaces.ac.cn/archives/11848>*

***更详细的转载事宜请参考：***[《科学空间FAQ》](https://spaces.ac.cn/archives/6508#%E6%96%87%E7%AB%A0%E5%A6%82%E4%BD%95%E8%BD%AC%E8%BD%BD/%E5%BC%95%E7%94%A8)
...
![image](https://spaces.ac.cn/usr/themes/geekg/payment/wx.png)

微信打赏

![image](https://spaces.ac.cn/usr/themes/geekg/payment/zfb.png)
...
苏剑林. (Aug. 04, 2026). 《简单谈谈K3的MoE和Attention 》\[Blog post\]. Retrieved from <https://spaces.ac.cn/archives/11848>
...
        year={2026},\
        month={Aug},\
        url={\url{https://spaces.ac.cn/archives/11848}},\
}

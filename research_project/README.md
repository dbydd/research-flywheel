...
research_project/
  ...
  <slug>/
    ...
    packs/G<n>.md                        # qa 提包（G2/G3 gate 包）
    gates/G<n>.decision.md               # 关口判词（chair 执笔）
    gates/G<n>-referee-<1|2|3>.md        # 独立意见书（席位号由 chair 分发点名）
```

run-id 全局唯一：`<slug>--r<round>`，账记 registry.json。过程件（lean/、measured/、run-log.md、header-snapshot.md）落 `runs/<run-id>/`。
...
- **§1 假设与理论论据** — speculator 主笔，theorist 补形式化小节（Lean 文件路径+陈述↔论据对照表）。
...
- G1 开题批准：examiner 判词三值 `pass`（开工令→theorist）/ `revise`（打回 pi，任务保持进行中）/ `fail`（planner 归案）。G1 打回满 3 次改判 fail。
- G2 中期验收：qa 提包 `packs/G2.md`，chair+referee×3 合议 `continue|rectify|stop`；rectify 连续满 3 轮转 stop。
- G3 末期验收：qa 提包 `packs/G3.md`（含 §2 全部结果节+约束核验+断言对表），chair+referee×3 依 §3 判 `accept|reject`；accept→planner 记档开新题。
